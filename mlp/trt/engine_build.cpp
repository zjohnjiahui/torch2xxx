...
IScaleLayer* addBatchNorm1d(INetworkDefinition *network, std::map<std::string, Weights>& weightMap,
    ...
    IScaleLayer* scale_1 = network->addScale(input, ScaleMode::kUNIFORM, shift, scale, power);
    assert(scale_1);
    return scale_1;
}
...
ICudaEngine *createEngine(unsigned int maxBatchSize, IBuilder *builder, IBuilderConfig *config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../mlp_bn.wts");

    // Create an empty network
    INetworkDefinition *network = builder->createNetworkV2(0U);

    // Create an input with proper *name
    ITensor *data = network->addInput("data", dt, Dims3{1, 1, 784});  // 28x28
    assert(data);


    IFullyConnectedLayer *fc1 = network->addFullyConnected(*data, 256,
                                                           weightMap["mlp.0.weight"],
                                                           weightMap["mlp.0.bias"]);
    IScaleLayer* scale1 = addBatchNorm1d(network, weightMap, *fc1->getOutput(0), "mlp.1", 1e-5);
    IActivationLayer* act1 = network->addActivation(*scale1->getOutput(0), ActivationType::kRELU);

    IFullyConnectedLayer *fc2 = network->addFullyConnected(*act1->getOutput(0), 128,
                                                           weightMap["mlp.3.weight"],
                                                           weightMap["mlp.3.bias"]);
    IScaleLayer* scale2 = addBatchNorm1d(network, weightMap, *fc2->getOutput(0), "mlp.4", 1e-5);
    IActivationLayer* act2 = network->addActivation(*scale2->getOutput(0), ActivationType::kRELU);

    IFullyConnectedLayer *fc3 = network->addFullyConnected(*act2->getOutput(0), 10,
                                                           weightMap["mlp.6.weight"],
                                                           weightMap["mlp.6.bias"]);
    ...
    // Set workspace size
    config->setMaxWorkspaceSize(1 << 21); // 10M
    // Set inference type
#if USE_FP16
