...
const int INPUT_SIZE = 784; // input data length in an  inference 28x28
const int OUTPUT_SIZE = 10; // output data length in an inference
...
void performInference() {
    ...
    cv::Mat img0 = cv::imread("../0.jpg", cv::IMREAD_GRAYSCALE);
    uchar* Img0Data = img0.data;
    ...
    for(int batch=0;batch<BATCH_SIZE;++batch)
    {
        ...
        {
            data[batch*INPUT_SIZE+i] = (imgData[batch][i]-127.5)/127.5;
            //std::cout << data[i] << " " ;
        }
    }
