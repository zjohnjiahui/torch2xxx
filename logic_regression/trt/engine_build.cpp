...
ICudaEngine *createLinearEngine(unsigned int maxBatchSize, IBuilder *builder, IBuilderConfig *config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../../logic_regression.wts");

    // Create an empty network
    INetworkDefinition *network = builder->createNetworkV2(0U);

    // Create an input with proper *name
    ITensor *data = network->addInput("data", dt, Dims3{1, 1, 2});
    assert(data);
    ...
    // Add layer for sigmoid
    IActivationLayer* act1 = network->addActivation(*fc1->getOutput(0), ActivationType::kSIGMOID);
    act1->getOutput(0)->setName("out");
    network->markOutput(*act1->getOutput(0));
...
void performSerialization() {
    ...
    std::ofstream p("../logic.engine", std::ios::binary);
    if (!p) {
    ...
    std::cout << "\n\tRun inference using `./logic -d`" << std::endl;
