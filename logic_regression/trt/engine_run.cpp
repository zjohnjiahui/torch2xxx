...
const int INPUT_SIZE = 2; // input data length in an  inference
const int OUTPUT_SIZE = 1; // output data length in an inference
...
void performInference() {
    ...
    std::ifstream file("../logic.engine", std::ios::binary);
    if (file.good()) {
    ...
    int scale = 8; // preprocess scale, decided in training period
    data[0] = 3.0; data[1] = 3.0;
    data[2] = 6.0; data[3] = 6.0;
    ...
    {
        std::cout << "\t" << data[2*i] << " " << data[2*i+1] << std::endl;
    }
