import torch
from logic_regression_v2 import LogicRegression
import struct

model_name = 'logic_regression'  # change to other to export other model
scale = 8  # for preprocess
...
def model_test(model, x):
    var = torch.tensor([[x]], dtype=torch.float32).to('cuda:0') / scale
